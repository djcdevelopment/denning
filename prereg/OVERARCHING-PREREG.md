...
- **RESULT (on-rig, 2026-06-19):** ✅ **SUPPORTED** — a lifetime-class slot-arena over real `llama-server` KV beats LRU by **+32.3% goodput** (95% CI [20.2, 44.4], 10 reps; 10/10 seeds; cache miss = ~2.8 s real re-prefill). Meets the criterion (≥20%, ≥10 reps, CI excludes 0). See [`../results/H4-arena-onrig-20260619.md`](../results/H4-arena-onrig-20260619.md). Caveats: conversation-grained (not block-grained); LRU baseline; constructed hot/cold workload. *(Prediction above unchanged.)*
