...
**Fabric-less.** Intel states plainly that "Arc does not support GPU-to-GPU connection"; multi-GPU P2P exists only on Linux (Project Battlematrix). On Windows, a card-to-card KV transfer is host-bounced — we measure **0.47×** of a host→VRAM copy. So multi-card serving is replica-per-card, not tensor-parallel-over-fabric; KV does not flow freely between GPUs. Yet replica-per-card scales *linearly*: two cards deliver **1.96×** aggregate decode with no steady-state contention (each replica streams its own resident weights), so the box serves ~2× the concurrent agent sessions by replication: the goodput knee doubles from **8 to 16** SLO-meeting sessions — the workload it is built for.
