...
| `B_pcie` | PCIe 4.0 x8 effective | **~13.9 GB/s asymptotic — MEASURED clean on Card B** (matches x8 datasheet; small-block 1.4–7 GB/s) |
| `B_dq(s)` | dequant-kernel output throughput, scheme s | **MEASURED: isolated 168/94, CONTENDED 48/41 (int8/int4) GB/s. R2 HOLDS isolated AND under saturating load — clears the threshold by 1.7–2.2× contended.** |
| `B_c2c` | card→card (host-bounced; no P2P on Windows) | **6.48 GB/s — MEASURED** (≈0.47× B_pcie → R3 confirmed) |
